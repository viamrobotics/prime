<script>
import Key from './key.svelte';
</script>

<p>Getting started:</p>

<ul class="flex !list-disc flex-col gap-2 pt-2">
  <li class="ml-4">Click on the map to add an obstacle.</li>
  <li class="ml-4">Click on an obstacle to select it.</li>
  <li class="ml-4">Drag an obstacle to move it.</li>
  <li class="ml-4">
    Hold <Key>shift</Key> and drag on the map to draw an obstacle.
  </li>
  <li class="ml-4">
    Hold <Key>command</Key> and drag an obstacle up or down to rotate it.
  </li>
  <li class="ml-4">
    Hold <Key>option</Key> and drag an obstacle up or down to scale it.
  </li>
</ul>
