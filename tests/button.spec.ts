...
test('Renders a button in the style of inverse primary if the variant is specified as inverse primary', async ({ page }) => {
  ...
  await expect(inversePrimaryButton.locator('button')).toHaveClass(/border-gray-9/)
  await expect(inversePrimaryButton.locator('button')).toHaveClass(/text-white/)
})
...
test('Renders a button in the style of success if the variant is specified as success', async ({ page }) => {
  ...
  await expect(successButton.locator('button')).toHaveClass(/border-success-fg/)
...
test('Renders a button in the style of danger if the variant is specified as danger', async ({ page }) => {
  ...
  await expect(dangerButton.locator('button').first()).toHaveClass(/bg-danger-fg/)
  await expect(dangerButton.locator('button').first()).toHaveClass(/border-danger-fg/)
  await expect(dangerButton.locator('button').first()).toHaveClass(/text-white/)
})
...
test('Renders a button in the style of outline danger if the variant is specified as outline danger', async ({ page }) => {
  ...
  await expect(outlineDangerButton.locator('button')).toHaveClass(/bg-danger-bg/)
  await expect(outlineDangerButton.locator('button')).toHaveClass(/border-danger-bg/)
  await expect(outlineDangerButton.locator('button')).toHaveClass(/text-danger-fg/)
})

test('Renders a button as disabled if the button has a disabled attribute of true', async ({ page }) => {
...
test('Renders an icon within the button/next to the label if an icon attribute is specified', async ({ page }) => {
  const iconWithinButton = page.getByRole('button', { name: 'Icon Within Button Label' })
