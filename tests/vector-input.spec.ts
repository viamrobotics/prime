...
test('Renders the default vector input with x, y, z placeholders, a step of 1, and input type text', async ({ page }) => {
  ...
  const xPlaceholderValue = await vectorInputDefault.locator('input').nth(0).getAttribute("placeholder")
  expect(xPlaceholderValue).toBe('x')
  const yPlaceholderValue = await vectorInputDefault.locator('input').nth(1).getAttribute("placeholder")
  expect(yPlaceholderValue).toBe('y')
  const zPlaceholderValue = await vectorInputDefault.locator('input').nth(2).getAttribute("placeholder")
  ...
  // input type tests
  const xTypeValue = await vectorInputDefault.locator('input').nth(0).getAttribute("type")
  expect(xTypeValue).toBe('text')
  const yTypeValue = await vectorInputDefault.locator('input').nth(1).getAttribute("type")
  expect(yTypeValue).toBe('text')
  const zTypeValue = await vectorInputDefault.locator('input').nth(2).getAttribute("type")
  expect(zTypeValue).toBe('text')

});

test('Renders three inputs with type text if number type specified', async ({ page }) => {
  const vectorInputNumType = page.getByTestId('vector-input-type-number')

  await expect(vectorInputNumType).toBeVisible()

  const xTypeValue = await vectorInputNumType.locator('input').nth(0).getAttribute("type")
  expect(xTypeValue).toBe('text')
  const yTypeValue = await vectorInputNumType.locator('input').nth(1).getAttribute("type")
  expect(yTypeValue).toBe('text')
  const zTypeValue = await vectorInputNumType.locator('input').nth(2).getAttribute("type")
  expect(zTypeValue).toBe('text')

});

test('Renders three inputs with type number if integer type specified', async ({ page }) => {
  const vectorInputTypeInteger = page.getByTestId('vector-input-type-integer')

  const xTypeValue  = await vectorInputTypeInteger.locator('input').nth(0).getAttribute("type")
  expect(xTypeValue ).toBe('number')
  const yTypeValue = await vectorInputTypeInteger.locator('input').nth(1).getAttribute("type")
  expect(yTypeValue).toBe('number')
  const zTypeValue = await vectorInputTypeInteger.locator('input').nth(2).getAttribute("type")
...
test('When label is specified, render label to the left of inputs', async ({ page }) => {
  ...
  const labelText = await vectorInputLabelLeft.getByRole('paragraph').textContent()
  expect(labelText).toBe('Label Left')

  // Confirm label is to the left of x input (x input is to the right of label)
  const xInputPlaceholder = (await page.locator("input:right-of(:text('Label Left'))").nth(1).getAttribute('placeholder'))
...
test('When step is specified, on arrow up, confirm input incremented to the correct value', async ({ page }) => {
...
test('When dimensions specified as 3, render 3 inputs with x, y, z placeholders', async ({ page }) => {
  ...
  const xPlaceholderValue = await vectorInputDimensions3.locator('input').nth(0).getAttribute("placeholder")
  expect(xPlaceholderValue).toBe('x')
  const yPlaceholderValue = await vectorInputDimensions3.locator('input').nth(1).getAttribute("placeholder")
  expect(yPlaceholderValue).toBe('y')
  const zPlaceholderValue = await vectorInputDimensions3.locator('input').nth(2).getAttribute("placeholder")
  expect(zPlaceholderValue).toBe('z')

});

test('When dimensions specified as 4, render 4 inputs with x, y, z, w placeholders', async ({ page }) => {
  ...
  const xPlaceholderValue = await vectorInputDimensions4.locator('input').nth(0).getAttribute("placeholder")
  expect(xPlaceholderValue).toBe('x')
  const yPlaceholderValue = await vectorInputDimensions4.locator('input').nth(1).getAttribute("placeholder")
  expect(yPlaceholderValue).toBe('y')
  const zPlaceholderValue = await vectorInputDimensions4.locator('input').nth(2).getAttribute("placeholder")
  expect(zPlaceholderValue).toBe('z')
  const wPlaceholderValue = await vectorInputDimensions4.locator('input').nth(3).getAttribute("placeholder")
