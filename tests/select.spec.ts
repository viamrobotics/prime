...
import { getCustomEventParam, waitForCustomEventWithParam } from './lib/helper.ts';

test.beforeEach(async ({page}) => {
  await page.goto('/select-test.html')
})

test('Given an options attribute, on select the options should be visible and clicking an option should update the input', async ({ page }) => {
...
test('Given a select with a placeholder, that should be visible in the input', async ({page}) => {
  const select = page.getByTestId('basic-select')
  await expect(select).toBeVisible()

  await expect(select.locator('input').first()).toHaveAttribute('placeholder', 'All')
})

test('Setting withbutton to true, with a button text and icon, the text and icon should be rendered as a final element', async ({page}) => {
...
test('If a select component has disabled=True the element should be disabled', async ({page}) => {
...
test('If the select component has readonly value, then it should be readonly', async ({page}) => {
...
test('If the select has attribute exact, the user can only type a value from the dropdown', async ({page}) => {
...
test('No state but tooltip applied should render info icon with tooltip text', async ({page}) => {
...
test('Given info state and tooltip value, there should be an info tooltip next to the select label', async ({page}) => {
...
test('Given warn state and tooltip value, there should be a warn tooltip next to the select label', async ({page}) => {
...
test('Given error state and tooltip value, there should be an error tooltip next to the select label', async ({page}) => {
...
test('When there is no sort on the dropdown, the options should be rendered in the order they are passed into the element', async ({page}) => {
  const select = page.getByTestId('basic-select')
  await expect(select).toBeVisible()
...
test('When there is a reduce sort option, the elements should reduce on matching input', async ({page}) => {
  ...
  await expect(optionsContainer.locator('label').first()).toHaveText('one')
  await expect(optionsContainer.locator('label').last()).toHaveText('one chicken')
})

test('On pressing enter over an option, that option should be selected', async ({ page }) => {
  const select = page.getByTestId('basic-select')
  await expect(select).toBeVisible()
