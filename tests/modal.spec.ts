...
test('Renders title and message', async ({ page }) => {
  ...
  await expect(modalOpen.getByText('lived by the sea and frolicked in the autumn mist in a land called Honah Lee.')).toBeVisible()
});
...
test('Hides when overlay behind the modal is focused and "Enter" key is pressed', async ({ page }) => {
  await page.goto('/modal-test.html')
...
test('Hides when "x" close icon is focused and "Enter" key is pressed', async ({ page }) => {
  await page.goto('/modal-test.html')
...
test('Renders child element in modal body', async ({ page }) => {
  ...
  // Slot
  const modalOpen = page.getByTestId('modal-open')
  await expect(modalOpen.getByText("Bring him strings and sealing wax and other fancy stuff")).toBeVisible()
});
