...
test.beforeEach(async ({ page }) => {
  await page.goto('/playground/collapse-test.html');
});

test('Confirms main title displays in collapse element if title attribute has been specified', async ({
  page,
}) => {
...
test('Collapse renders open with chevron in closed position if open attribute specified as true', async ({
  page,
}) => {
...
test('Collapse renders closed with chevron in closed position if open attribute specified as false', async ({
  page,
}) => {
...
test('If collapse is in closed position, confirm it opens after click and vice versa', async ({
  page,
}) => {
  const collapse = page.getByTestId('collapse-default');
...
test('Confirms collapse with breadcrumbs and badge displays properly', async ({
  page,
}) => {
  ...
  const bbExpandText =
    'How do you tell how good bread is without tasting it? Not the smell, not the look, but the sound of the crust. Listen. Symphony of crackle.';
