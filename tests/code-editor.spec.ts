...
test('Renders Javascript code editor that is valid', async ({ page }) => {
  const validJavascriptEditor = page.getByTestId(
    'code-editor-javascript-valid'
  );
...
test('Renders Javascript code editor that is invalid', async ({ page }) => {
  const invalidJavascriptEditor = page.getByTestId(
    'code-editor-javascript-invalid'
  );
  await expect(invalidJavascriptEditor).toHaveAttribute(
    'language',
    'javascript'
  );
  await expect(invalidJavascriptEditor).toHaveAttribute(
...
test('Renders typescript code editor that is valid', async ({ page }) => {
  const validTypescriptEditor = page.getByTestId(
    'code-editor-typescript-valid'
  );
...
test('Renders typescript code editor that is invalid', async ({ page }) => {
  const invalidTypescriptEditor = page.getByTestId(
    'code-editor-typescript-invalid'
  );
  await expect(invalidTypescriptEditor).toHaveAttribute(
    'language',
    'typescript'
  );
...
test('If a readonly attribute is not set, the editor should be editable', async ({
  ...
  await expect(
    validJSONEditor.locator('.message', {
      hasText: 'Cannot edit in read-only editor',
    })
  ).not.toBeVisible();
...
test('If a readonly attribute is set, then the editor should be not editable', async ({
  page,
}) => {
  ...
  await expect(
    editor.locator('.message', { hasText: 'Cannot edit in read-only editor' })
  ).toBeVisible();
...
test('Render a minimap if the minimap attribute is set to true', async ({
  ...
  const width = Number(
    await editor.locator('.minimap-decorations-layer').getAttribute('width')
  );
...
test('Render a diff code editor if there is a previous attribute and variant diff', async ({
  ...
  await expect(editor.locator('.editor.original')).toHaveAttribute(
    'data-mode-id',
    'json'
  );
  await expect(editor.locator('.editor.modified')).toHaveAttribute(
    'data-mode-id',
    'json'
  );
